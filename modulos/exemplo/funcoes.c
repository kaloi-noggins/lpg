...
int *divisores(int x)
{
    int *divisores, *temp, count, divisor = 0;
    ...
    for (count = 0; count < x; count++)
        temp[count] = 0; //Checando os divisores de 'x' e colocando no vetor temp
...
void executa(int opcao)
{
    ...
    case 11:
        ...
        printf("Numero: ");
        scanf("%d", &x);
        printf("Divisores de %d: ", x); //Exibindo o vetore de numeros
        div = divisores(x);
        for (y = 0; div[y - 1] != x; y++)
            printf("%3d ", div[y]);
        printf("\n\n");
